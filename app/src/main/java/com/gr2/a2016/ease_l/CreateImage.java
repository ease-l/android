...
public class CreateImage extends Activity implements View.OnClickListener{
RequestQueue queue;
    CreateImage context;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((Button)findViewById(R.id.backim)).setOnClickListener(this);
        ((Button)findViewById(R.id.sendimg)).setOnClickListener(this);
        ...
        if(v.getId() == R.id.backim){
            ...
            if(!name.equals("")){
                ...
                try {
                    ...
                    object.put("Url",((EditText)findViewById(R.id.urlimage)).getText().toString());
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                if (!id.equals(" ")){
                    JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, NetworkAdreses.GET_PROJECT_BY_ID + id + "/image", object, new Response.Listener<JSONObject>() {
                        @Override
                    ...
                    JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, NetworkAdreses.GET_ALL_IMAGE, object, new Response.Listener<JSONObject>() {
                        @Override
                        public void onResponse(JSONObject jsonObject) {
                            Toast.makeText(context,"успех!!",Toast.LENGTH_LONG).show();
                            context.setResult(RESULT_OK);
